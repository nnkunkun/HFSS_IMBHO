function main_parameter_sweep()
    ...
    for sim_idx = 1:total_simulations
        ...
        cleanup_project_lock_v2(config.hfss_project);
        [success, result_files] = run_single_simulation_v2(current_params, export_types, simulation_id, config);

        if success
            combined_data = process_exported_data_v2(result_files, current_params, sim_idx, simulation_id);
        ...
        results_log = record_simulation_results_v2( ...
    ...
    combine_all_results_v2(results_log, config.results_dir);
...
function combined_data = process_s11_exported_data(result_files, params, sim_idx, simulation_id)
    combined_data = process_exported_data_v2(result_files, params, sim_idx, simulation_id);
end

function combine_all_s11_results(results_log, results_dir)
    combine_all_results_v2(results_log, results_dir);
end

function results_log = record_s11_simulation_results(results_log, idx, simulation_id, params, result_files, success)
    results_log = record_simulation_results_v2(results_log, idx, simulation_id, params, result_files, success);
end

function [success, result_files] = run_single_s11_simulation(params, export_types, simulation_id, config)
    [success, result_files] = run_single_simulation_v2(params, export_types, simulation_id, config);
end
...
function combined_data = process_exported_data_v2(result_files, params, sim_idx, simulation_id)
    combined_data = table();
...
function combine_all_results_v2(results_log, results_dir)
    all_data = table();
...
function results_log = record_simulation_results_v2(results_log, idx, simulation_id, params, result_files, success)
    param_names = fieldnames(params);
    for i = 1:numel(param_names)
...
function [success, result_files] = run_single_simulation_v2(params, export_types, simulation_id, config)
    success = false;
    result_files = struct();

    try
        run_hfss_com_v2(params, export_types, simulation_id, config);
...
function run_hfss_com_v2(params, export_types, sim_id, config)
    max_retries = 3;
    retry_count = 0;
    ...
    while retry_count < max_retries
        try
            ...
            cleanup_project_lock_v2(config.hfss_project);
            project_path = create_temp_project_copy_v2(config, sim_id);
            ...
            for i = 1:size(export_types, 1)
                ...
                if ~file_created
                    error('Export file was not created for %s.', export_name);
                end
            end
            ...
            cleanup_temp_project_copy_v2(temp_project_path, config);
            temp_project_path = '';
            fprintf('仿真流程完成\n');
            return;
        catch ME
...
function apply_parameter_values_v2(design, parameter_values, unit_suffix)
...
function temp_project_path = create_temp_project_copy_v2(config, sim_id)
    ...
    cleanup_project_lock_v2(temp_project_path);
end

function cleanup_temp_project_copy_v2(temp_project_path, config)
    if nargin < 2 || ~isfield(config, 'keep_temp_projects') || ~config.keep_temp_projects
        try_delete_file_v2(temp_project_path);
        try_delete_file_v2([temp_project_path, '.lock']);
        try_delete_dir_v2([temp_project_path, 'results']);
    end
end

function cleanup_project_lock_v2(project_path)
    lock_path = [project_path, '.lock'];
    if ~exist(lock_path, 'file')
        return;
    end

    desktop_pid = read_lock_process_id_v2(lock_path);
    if isempty(desktop_pid) || ~is_process_running_v2(desktop_pid)
        try_delete_file_v2(lock_path);
    end
end

function pid = read_lock_process_id_v2(lock_path)
    pid = [];
    try
...
function tf = is_process_running_v2(pid)
    tf = false;
    try
...
function try_delete_file_v2(file_path)
...
function try_delete_dir_v2(dir_path)
...
function run_hfss_com(params, export_types, sim_id, config)
    ...
    while retry_count < max_retries
        try
            ...
            % 获取设计
            try
                design = project.SetActiveDesign('EllipticalProbe_ATK');
            catch
                design = project.GetActiveDesign();
            end
            fprintf('设计获取成功: %s\n', design.GetName());
