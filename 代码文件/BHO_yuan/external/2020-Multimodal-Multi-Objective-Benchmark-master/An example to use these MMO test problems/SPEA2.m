...
VarMin=xl;           % Decision Variables Lower Bound  % 由VarMin=xl'改成了VarMin=xl;（2018.7.3.23.05）;   
VarMax=xu;           % Decision Variables Upper Bound  % 由VarMax=xu‘改成了VarMax=xu;（2018.7.3.23.05）; 
...
for i=1:nPop
    pop(i).Position=VarMin+(VarMax-VarMin).*rand(1,nVar);% 由rand(nVar,1)改成rand(1，nVar)（2018.7.3.23.08）;
...
%**添加于2018.7.7.18.00 处理有时取出来ps或pf是1*popsize*维度而不是popsize*维度的情况
if size(PF(1).Position,1)==1 %如果自变量行数是1
...
if size(PF(1).Cost,1)==1 %如果目标值行数是维度，列数是1
    pf=cat(1,PF.Cost);
else
    pf=cat(2,PF.Cost);
    pf=pf';
end
%**end 添加于2018.7.7.18.00

% ps=[PF.Position];%删除于2018.7.7.18.00
% pf=[PF.Cost];
% ps=ps';
% pf=pf';
end
