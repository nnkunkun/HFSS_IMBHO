function metrics = compute_mo_metrics_hfss(PopDec, PopObj, refPF, varargin)
% 中文说明：
% 统一计算 HFSS 工程多目标实验的 HV、IGD、Spread、Spacing 和非支配解个数。

opts.hvSamples = 100000;
if ~isempty(varargin)
...
metrics = struct('HV', nan, 'IGD', nan, 'Spread', nan, 'Spacing', nan, 'NDS', 0);
if isempty(PopObj) || isempty(refPF) || size(PopObj, 2) < 2
    return;
end

validPop = all(isfinite(PopObj), 2);
validRef = all(isfinite(refPF), 2);
PopDec = PopDec(validPop, :);
PopObj = PopObj(validPop, :);
refPF = refPF(validRef, :);
if isempty(PopObj) || isempty(refPF)
    return;
end
...
if isempty(PopObj) || size(PopObj, 1) < 2
    score = nan;
    return;
end
